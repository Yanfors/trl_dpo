...
class ScriptArguments:
    model_name: Optional[str] = field(default="google/gemma-2b", metadata={"help": "the model name"})
...
if torch.cuda.is_available():
    ...
    # 获取当前GPU的属性，例如名称、总显存、已使用显存等
    props = torch.cuda.get_device_properties(device_id)
    
    # 打印GPU信息
    print(f"Device {device_id}: {props.name}")
    print(f"Total memory: {props.total_memory / 1024 ** 2} MB")
    print(f"Available memory: {torch.cuda.memory_allocated(device_id)} bytes")
    print(f"Used memory: {torch.cuda.max_memory_allocated(device_id)} bytes")
    
print(f'now {torch.cuda.memory_allocated(device=0)} B')
print(f'max {torch.cuda.max_memory_allocated(device=0)} B')
...
print(f'now {torch.cuda.memory_allocated() / 1024 ** 2} MB')
print(f'max {torch.cuda.max_memory_allocated() / 1024 ** 2} MB')
